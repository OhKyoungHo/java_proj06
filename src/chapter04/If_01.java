...
public class If_01 {
	...
	public static void main(String[] args) {
		/* 
		 ...
		 - if (조건1){				// 조건 1이 참일때, 실행문 1을 실행
		 	  실행문 1 ;
		   } else if(조건2){			// 조건 2가 참일때, 실행문 2를 실행
		   	  실행문 2 ;
		   } else if(조건3){ 			// 조건 3가 참일때, 실행문 3을 실행
		   	  실행문 3 ;
		   }
		   .....
		...
		System.out.println("============================");
		if ( 5 < 3 ) 
			System.out.println("오늘은 비가 옵니다.");	// 중괄호 생략, 해당 if 조건이 false인 경우, 해당 실행문만 if 조건을 받아서 출력되지 않는다. 
		...
		*/	
		...
		//		- 기본 자료형(8가지) : 초기값을 할당하지 않으면 오류
					// boolean(참, 거짓), byte, short, int, long(정수), float, double(실수), char
		...
		if (c >= 90) {								// false
			...
			System.out.println("B 학점입니다.");		// 실행문 실행 후 if문을 빠져 나옴
		}else if ( c >= 70 ) {
