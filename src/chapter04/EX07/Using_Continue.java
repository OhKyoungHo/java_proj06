...
public class Using_Continue {
	...
	public static void main(String[] args) {
		...
		for(int i=0 ; i<10 ; i++ ) {		// 0~9 : 10번
			...
			continue ;						//continue를 만나면 아래의 코드를 실행하지 않고 위의 코드로 가서 반복 실행
			//System.out.println("B");		//도달할 수 없는 코드
		}
		System.out.println("============================");
		for(int i=0 ; i<10 ; i++ ) {		// 0~9 : 10번
			System.out.println("A");
			if (i %2 == 0) {continue;} 		//i: 0, 2, 4, 6, 8일때 continue 실행
											//continue를 만나면 아래의 코드를 실행하지 않고 위의 코드로 가서 반복 실행
	...
	out:for(int i =0 ; i<5 ; i++) {			//5번 
		for(int k=0 ; k<5; k++) {			//5번
			if(k==3) {continue out;}
			System.out.println(i + "," + k);		//레이블 :(:이 점프를 의미)
		}
	}
	...
	out:for(int i =0 ; i<5 ; i++) {			//5번 
		for(int k=0 ; k<5; k++) {			//5번
			if(k==3) {
				i = 100;					//바깥쪽 for 문의 변수값을 증대시켜서 false 빠져나옴. 
				continue out;}
			System.out.println(i + "," + k);		//레이블 :(:이 점프를 의미)
		}
	}
